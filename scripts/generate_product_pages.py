...
def render_page(product, related, slug, landing_lookup=None):
    ...
    product_json = json.dumps(
        {
            "id": product.get("id"),
            "code": product.get("sourceCode") or product.get("id"),
            "title": title,
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .top-share-stack {{ display: flex; flex-direction: column; gap: 8px; pointer-events: auto; }}
    .icon-action {{ display: grid; place-items: center; width: 46px; height: 46px; border: 1px solid rgba(214, 216, 220, 0.72); border-radius: 8px; background: rgba(255, 255, 255, 0.92); color: #1f1f1f; box-shadow: 0 10px 22px rgba(0, 0, 0, 0.08); cursor: pointer; pointer-events: auto; touch-action: manipulation; user-select: none; -webkit-user-select: none; text-decoration: none; }}
    ...
    .share-feedback {{ position: fixed; left: 50%; bottom: calc(82px + env(safe-area-inset-bottom)); z-index: 40; width: max-content; max-width: calc(100vw - 32px); transform: translateX(-50%); padding: 11px 14px; border-radius: 8px; color: #fff; background: rgba(20, 24, 28, 0.94); box-shadow: 0 12px 30px rgba(0, 0, 0, 0.22); font-size: 14px; font-weight: 750; text-align: center; }}
...
<body class="product-page">
  ...
  <main class="page">
    <article class="product-shell">
      ...
      <div class="share-feedback" data-share-feedback role="status" aria-live="polite" hidden></div>
  ...
  <script>
    ...
    function trackedShareUrl(source, medium, campaign) {{
      ...
      url.searchParams.set("utm_content", product.code || product.id);
      return url.toString();
    }}
    ...
    async function copyShareText(shareData) {{
      const text = [shareData.text, shareData.url].filter(Boolean).join("\\n");
      try {{
    ...
    async function shareProductLink(shareData, button) {{
      ...
      if (copied) {{
        ...
        showShareFeedback("Текст и ссылка скопированы");
        return true;
      }}

      showShareFeedback("Не удалось открыть меню. Обновите страницу и попробуйте снова.");
      return false;
    }}

    function bindShareButton(button, makeShareData) {{
      if (!button) return;
      let lastTouchShareAt = 0;

      const runShare = async () => {{
        button.classList.add("active");
        window.setTimeout(() => button.classList.remove("active"), 650);
        await shareProductLink(makeShareData(), button);
      }};

      button.addEventListener("pointerup", (event) => {{
        if (event.pointerType !== "touch" && event.pointerType !== "pen") return;
        event.preventDefault();
        lastTouchShareAt = Date.now();
        void runShare();
      }});

      button.addEventListener("click", (event) => {{
        if (Date.now() - lastTouchShareAt < 800) {{
          event.preventDefault();
          return;
        }}
        void runShare();
    ...
    bindShareButton(document.querySelector("[data-share]"), () => ({{
        title: product.title,
        text: product.title,
        url: trackedShareUrl("share", "referral", "product_share"),
    }}));

    function browserSmmSession() {{
      for (let index = 0; index < localStorage.length; index += 1) {{
        const key = localStorage.key(index);
        if (!key?.startsWith("sb-") || !key.endsWith("-auth-token")) continue;
        try {{
          const value = JSON.parse(localStorage.getItem(key));
          const user = value?.user || value?.currentSession?.user || value?.session?.user;
          if (
            user?.app_metadata?.is_admin === true
            || user?.app_metadata?.is_smm === true
          ) return true;
        }} catch (_) {{}}
      }}
      return false;
    ...
    const smmShareButton = document.querySelector("[data-smm-share]");
    if (smmShareButton && browserSmmSession()) {{
      smmShareButton.hidden = false;
      bindShareButton(smmShareButton, () => {{
        const price = Number(product.price || 0).toLocaleString("ru-RU");
        const description = String(product.description || "").trim().slice(0, 180);
        const text = [product.title, `${{price}} сом`, description].filter(Boolean).join("\\n");
        return {{
          title: product.title,
          text,
          url: trackedShareUrl("instagram", "organic_social", "smm_product"),
        }};
      }});
    }}
